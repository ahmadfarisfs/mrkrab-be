...
func (ps *ProjectStore) CreateProject(name string, amount int, description string, status string) (model.Project, error) {
	ret := model.Project{Name: name, Amount: amount, Description: description, Status: status}
...
func (ps *ProjectStore) GetProjectDetails(projectID int) (model.Project, error) {
	prj := model.Project{}
	err := ps.db.Model(&model.Project{}).Where("id = ?", projectID).First(&prj).Error
	return prj, err
}

func (ps *ProjectStore) UpdateProject(id int, name string, amount int, description string, status string) error {
	prj, err := ps.GetProjectDetails(id)
	if err != nil {
		return err
	}
	prj.Name = name
	...
	prj.Status = status
	return ps.db.Model(&model.Project{}).Updates(&prj).Error
}

func (ps *ProjectStore) DeleteProject(id int) error {
	return ps.db.Delete(&model.Project{}, id).Error
...
// 		return nil, err
// 	}
// 	accountDet := model.FinancialAccount{}
// 	err = ps.db.Model(&model.FinancialAccount{}).Where("id = ?", accountID).First(&accountDet).Error
// 	if err != nil {
// 		return nil, err
// 	}
// 	subAccountsDet := []model.FinancialAccount{}
// 	err = ps.db.Model(&model.FinancialAccount{}).Where("parent_id is not null and parent_id = ?", accountDet.ID).
